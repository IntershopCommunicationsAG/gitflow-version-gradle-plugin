...
import com.intershop.gradle.gitflow.services.git.GitLocalService.Companion.HASHLENGTH
...
import com.intershop.gradle.gitflow.version.ScmBranchFilter
import com.intershop.gradle.gitflow.version.ScmVersionObject
import com.intershop.gradle.gitflow.version.VersionTag
 ...
 */
open class VersionService(val project: Project,
                          val scmExtension: ScmExtension) {
     ...
     */
    val scmService: RepositoryService by lazy {
        RepositoryService(project.projectDir, scmExtension)
    }
     ...
     */
     val defaultVersion : Version
        get() {
            return if(scmExtension.version.initialVersion.isNotEmpty()) {
                try {
                    Version.valueOf(scmExtension.version.initialVersion)
                } catch (pex: ParserException) {
                    (Version.Builder(scmExtension.version.versionType)).build()
                }
            } else {
                (Version.Builder(scmExtension.version.versionType)).build()
            }
     ...
     */
    val versionObject: ScmVersionObject by lazy {
        var rv: ScmVersionObject? = null

        if(scmService.tagName != "") {
            rv = ScmVersionObject(scmService.branchName,
                    Version.forString(scmService.tagName, scmExtension.version.versionType),
                    scmService.changed)
        } else {

            val headId = scmService.repository.resolve(scmService.revID)
    ...
    private fun calcVersion(): String {
        with(scmExtension.version) {
            if (!disableSCM) {
                val tempVersion: Version = preVersion
                val revIDExtension = scmRevExtension

                if (continuousRelease && revIDExtension.isNotEmpty() && ! scmService.changed) {
                ...
                if (scmService.branchType == BranchType.DETACHEDHEAD) {
                    val versionForDetachedHead = tempVersion.setBuildMetadata(revIDExtension)

                    return if (continuousRelease && !scmService.changed) {
     ...
     */
     fun getBranchFilter(type: BranchType): ScmBranchFilter {
        return ScmBranchFilter(localService.prefixes, type, scmService.branchName,
                scmService.branchType, versionExt.patternDigits)
    }
}
